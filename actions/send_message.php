...
// Check if there's a message or photo - ALLOW EMPTY for photo only
if (empty($message) && (!isset($_FILES['photo']) || $_FILES['photo']['error'] === UPLOAD_ERR_NO_FILE)) {
    http_response_code(400);
    echo json_encode(['error' => 'Message or photo is required', 'received_message' => $message, 'has_photo' => isset($_FILES['photo'])]);
    exit;
}
...
if (isset($_FILES['photo']) && $_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
    ...
    if (move_uploaded_file($file['tmp_name'], $filepath)) {
        chmod($filepath, 0644);
        $photo_url = 'assets/uploads/chat/' . $filename;
